...
async function SetRedisCacheValue() {
  try {
    await fetch(`${server}/redis-test`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ Id: redisKey.value, Value: redisValue.value })
      })
  } catch (err) {
    console.log(err);
  }
...
async function GetRedisCacheValue() {
  try {
    console.log(`${server}/redis-test?Id=${redisKey.value}`);
    const res = await fetch(`${server}/redis-test?Id=${redisKey.value}`);
    const data = await res.json();
    redisCacheValue.value = data.value;
  } catch (err) {
    console.log(err)
  }
}
...
<div style="width: 100%;height: 100%; display: flex;justify-content: center; margin-top: 1rem;">
  <div style="border: 1px solid; padding: 2rem;display: flex; flex-direction: column; gap:1rem;width: fit-content; ">
    ...
    <div style="display: flex; gap: 1rem; text-wrap: nowrap; justify-content: space-between; ">
      <button @click="SetRedisCacheValue">Set Redis Cache Value</button>
      <div style="width: 100%; display: flex;gap: 1rem; "><span>Key:</span>
        <input style="width: 100%;" v-model="redisKey" />
      </div>
      <div style="display: flex;gap:1rem; width: 100%; text-align: right;"><span>Value:</span>
        <input style="width: 100%;text-align: right;" v-model="redisValue" />
      </div>
    </div>
    <div style="display: flex; gap: 1rem; text-wrap: nowrap; justify-content: space-between; ">
      <button @click="GetRedisCacheValue">Get Redis Cache Value</button>
      <div style="display: flex; gap:1rem;">Redis Cache Value is:
        <span :v-model="redisCacheValue">{{ redisCacheValue }}</span>
      </div>
    </div>
    <div>
